...
import { createVote, getVoteById, updateVote, Vote } from "~/models/vote.server";
...
export const loader: LoaderFunction = async ({
    ...
    const vote = await getVoteById(voteInt);
    if (!vote) {
        return json({ error: "Голосование не найдено" }, { status: 404 });
    }
    return { vote };
}
...
export const action: ActionFunction = async ({
    ...
    const voteId = params.voteid;
    const voteInt = parseInt(voteId || "0");
    await updateVote(voteInt, name, url, title, active === "on" ? true : false);
    return redirect("/admin/settings/vote/");
};

const Edit = () => {
    const data = useLoaderData();
    const vote:Vote = data.vote;
    const actionData = useActionData() as ActionData;

    return (
        <Form method="post">
            <div className="container mx-auto">
                <h1 className="text-gray-900 dark:text-gray-300 text-center">Редактирование голосования</h1>
