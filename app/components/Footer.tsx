    ...
    return(
        <footer className="p-4 sm:p-6 container mx-auto">
            <div className="md:flex md:justify-between md:max-w-4xl md:mx-auto">
                ...
                <div className="flex flex-row justify-between md:w-2/3 md:pl-24">
                    <div className="w-full">
                        <h2 className="mb-6 text-sm text-white uppercase font-semibold">Голосуйте за нас</h2>
                        <ul className="text-gray-600 dark:text-gray-400">
                            {settings.Vote.map((vote, index) => (
                                <li key={index} className="mb-2 last:mb-0">
                                    <a href={vote.url} className="hover:underline text-[##737373] hover:text-gray-400">{vote.title}</a>
                                </li>
                            ))}
                        </ul>
                    </div>
                    <div className="float-right w-full">
                        <h2 className="mb-6 text-sm text-white uppercase font-semibold">Ссылки</h2>
                        <ul className="text-gray-600 dark:text-gray-400">
                            {settings.Link.map((link, index) => (
                                <li key={index} className="mb-2 last:mb-0">
                                    <a href={link.url} className="hover:underline text-[##737373] hover:text-gray-400">{link.title}</a>
                                </li>
                            ))}
                        </ul>
                    </div>
            ...
            <div className="sm:flex sm:items-center sm:justify-between md:max-w-4xl md:mx-auto">
                            ...
                            <Interweave content={settings.yandex} className={"flex justify-center md:block"}/>
