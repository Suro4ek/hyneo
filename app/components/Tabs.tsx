                    ...
                    {items.map((category) => (
                        ...
                        >
                            {category.items.map((item) => (
                                <div key={item.id} className="my-1 px-1 w-full xl:1/3 md:w-1/3 lg:my-4 lg:px-4 lg:w-1/3 xl:w-1/4 2xl:w-1/5 mt-4 group" >
                                    <div className={classNames( "overflow-hidden shadow-2xl text-white h-full text-center text-2xl transition duration-[800ms] transform bg-tab-card hover:bg-tab-card-hover")} onClick={() => {
                                        setbuyItem(item);
                                        setOpen(true);
                                    }
                                    }>
                                        <div className="border-b-[1px] border-white p-2 text-base">
                                            {item.name}
                                        </div>
